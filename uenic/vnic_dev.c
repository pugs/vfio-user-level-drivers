 ...
 *
 */
// #ident "$Id: vnic_dev.c 24831 2009-03-11 00:44:48Z varja $"
...
struct vnic_dev {
	void *priv;
	void *pdev;	/* handle for higher level */
...
struct vnic_dev *vnic_dev_register(struct vnic_dev *vdev,
	void *priv, void *pdev, void *va, int barlen)
{
	struct vnic_dev_bar *bar;
	...
	bar = kzalloc(sizeof(struct vnic_dev_bar), GFP_ATOMIC);
	if (!bar)
		return NULL;
	bar->vaddr = va;
	bar->len = barlen;
